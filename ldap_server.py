...
class LdapService():
    ...
    ldap_server = "ldap://192.168.233.146:389"  # host address
    ldap_ou = "security"  # organization unit
    ldap_group = "security-group"  # organization sub-group

    # admin domain
    LDAP_ADMIN_DN = "cn=admin,dc=messenger,dc=com"
    LDAP_ADMIN_PWD = ""
    ...
    def login(self, username, password):
        ...
        user_dn = "cn=" + self.username + ",cn=" + self.ldap_group + ",ou=" + \
            self.ldap_ou + ",dc=messenger,dc=com"

        print(user_dn)

        # base domain
        LDAP_BASE_DN = "cn=" + self.ldap_group + \
            ",ou=" + self.ldap_ou + ",dc=messenger,dc=com"
